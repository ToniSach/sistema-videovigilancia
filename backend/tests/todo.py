...
Ejecuta: python test_system_integrado.py
...
6. Envía alertas Telegram (pide token/chat_id al inicio)
"""
import sys
import os
import threading
...
def setup_telegram():
    """Pide credenciales Telegram y las guarda"""
    print("\n" + "="*60)
    print("CONFIGURACIÓN TELEGRAM")
    print("="*60)
    print("1. Crea un bot con @BotFather en Telegram")
    print("2. Obtén el token (ej: 123456:ABC-DEF1234ghI-klmn)")
    print("3. Escribe /start al bot")
    print("4. Obtén tu Chat ID (usa @userinfobot o similar)")
    print("="*60)
    ...
    print("✅ Configuración Telegram guardada")
...
def discover_and_setup_camera():
    ...
    if devices:
        ...
        selected = {
            ...
            "username": user,
            "password": pwd,
            "has_ptz": True,  # Asumir que tiene hasta verificar
            "has_audio": False,
            "has_leds": False,
            "is_dual_lens": False,
...
def main():
    ...
    ai_service.activate_ai(camera.id, mode="high_quality")  # Modo calidad para prueba
    
    ia_start_time = time.time()
    ia_duration = 300  # 5 minutos en segundos
    ...
    window_name = f"NVR Test - Cámara {camera.name}"
    ...
    try:
        while True:
            ...
            if frame_data:
                ...
                # Info en pantalla
                status_text = []
                status_text.append(f"Cámara: {camera.name}")
                status_text.append(f"IA: {'ACTIVA' if remaining > 0 else 'INACTIVA'} ({int(remaining)}s)")
                status_text.append("PTZ: WASD+Q/E | ESC para salir")
            ...
            # Capturar tecla (espera 30ms = ~30fps)
            ...
            if remaining <= 0 and ai_service._schedulers.get(camera.id):
                print("⏰ Tiempo de IA finalizado (5 minutos). Desactivando...")
